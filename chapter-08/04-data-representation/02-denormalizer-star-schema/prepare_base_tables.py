...
from config import get_output_visits_dir, get_output_date_dir, get_output_page_dir, get_input_visits_dir, \
    get_output_month_dir, get_output_quarter_dir, get_output_category_dir
...
if __name__ == "__main__":
    ...
    dim_date = (visits_dataset.selectExpr(
        ...
        'HASH(MONTH(event_time)) AS dim_month_id',
        'HASH(QUARTER(event_time)) AS dim_quarter_id',
        'YEAR(event_time) AS year'
    ).dropDuplicates())
    dim_date.write.mode('overwrite').format('delta').save(get_output_date_dir())

    dim_date_month = (visits_dataset.selectExpr(
        'HASH(MONTH(event_time)) AS month_id',
        'DATE_FORMAT(event_time, "MMM") AS month_short_name',
        'MONTH(event_time) AS month_number'
    ).dropDuplicates())
    dim_date_month.write.mode('overwrite').format('delta').save(get_output_month_dir())

    dim_date_quarter = (visits_dataset.selectExpr(
        'HASH(QUARTER(event_time)) AS quarter_id',
        'CONCAT("Quarter ", QUARTER(event_time)) AS quarter_short_name',
        'QUARTER(event_time) AS quarter_number'
    ).dropDuplicates())
    dim_date_quarter.write.mode('overwrite').format('delta').save(get_output_quarter_dir())

    dim_page = (visits_dataset.selectExpr(
        'HASH(page) AS page_id',
        'page AS page_name',
        'CONCAT("pages/", REPLACE(page, " ", "")) AS page_url',
        'HASH(CONCAT("cat_", page)) AS dim_page_category_id'
    ).dropDuplicates())
    dim_page.write.mode('overwrite').format('delta').save(get_output_page_dir())

    dim_page_category = (visits_dataset.selectExpr(
        'HASH(CONCAT("cat_", page)) AS page_category_id',
        'CONCAT("cat_", page) AS category_name',
        'CONCAT("categories/", category_name) AS category_url'
    ).dropDuplicates())
    dim_page_category.write.mode('overwrite').format('delta').save(get_output_category_dir())
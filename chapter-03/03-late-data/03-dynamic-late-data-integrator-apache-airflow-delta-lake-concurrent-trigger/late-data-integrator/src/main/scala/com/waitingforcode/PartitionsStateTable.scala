...
class PartitionsStateTable(sparkSession: SparkSession, baseDir: String) {
  ...
  def getPartitionsToBackfill(lastVersionPerPartition: Map[String, Long],
    ...
    import sparkSession.implicits._
    sparkSession.read.format("delta").load(TablePath).show(false)
  ...
  def markPartitionAsBeingProcessed(partition: String): Unit = {
    markPartitionsAsBeingProcessed(Seq(partition))
  }

  def markPartitionsAsBeingProcessed(partitions: Seq[String]): Unit = {
  ...
  def markPartitionsAsProcessed(partition: String, version: Long): Unit = {

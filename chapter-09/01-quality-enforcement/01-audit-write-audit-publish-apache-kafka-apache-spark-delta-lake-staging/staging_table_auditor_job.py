...
from config import get_visit_event_schema, get_staging_visits_table, get_valid_visits_table, get_error_table
...
if __name__ == "__main__":
    ...
    visits = (spark_session.readStream.format('delta')
              .option('maxBytesPerTrigger', 20000000)  # 20MB
              .table(get_staging_visits_table())
              .withColumn('is_valid', row_validation_expression)
              )


    def audit_dataset_and_write_to_output_table(visits_to_audit: DataFrame, batch_version: int):
        ...
        if should_write_to_valid_table:
            (visits_to_audit.filter('is_valid = TRUE').drop('is_valid').drop('batch_version')
    ...
    write_query = (visits.writeStream
                   ...
                   .option('checkpointLocation',
                           '/tmp/dedp/ch09/01-quality-enforcement/01-audit-write-audit-publish-apache-kafka-apache-spark-delta-lake-staging/auditor/checkpoint')
                   .outputMode('append').foreachBatch(audit_dataset_and_write_to_output_table).start())

...
from visit_mapper import VisitAuditor
...
visits: DataStream = data_source.map(VisitAuditor(invalid_data_output), Types.STRING())

kafka_sink_valid_data: KafkaSink = create_kafka_sink('localhost:9094', 'valid_visits')

kafka_sink_invalid_data: KafkaSink = create_kafka_sink('localhost:9094', 'invalid_visits')

visits.get_side_output(invalid_data_output).sink_to(kafka_sink_invalid_data)
visits.sink_to(kafka_sink_valid_data)

env.execute('Visits auditor')

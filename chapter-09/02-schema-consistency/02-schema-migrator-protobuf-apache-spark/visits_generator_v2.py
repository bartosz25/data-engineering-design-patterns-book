...
from protobuf_output.python.definitons import visit_v2_pb2

if __name__ == '__main__':
    def visit(event_time: datetime, visit_id='id_1', page='home', user_id='user_1'):
        visit_to_write = visit_v2_pb2.VisitV2()
        visit_to_write.visit_id = visit_id
        visit_to_write.page = page
        visit_to_write.event_time.FromDatetime(event_time)
        visit_to_write.referral = f'origin {user_id} {visit_id}'
        visit_to_write.user_details.id = user_id
        visit_to_write.user_details.ip = f'1.1.1.{user_id}'
        visit_to_write.user_details.login = f'login {user_id}'
        visit_to_write.user_details.is_connected = True
